...
class ServiceWorkerRegistration extends events.EventEmitter {
    ...
    x_getWorkerSource () { // eslint-disable-line camelcase
...
class ServiceWorkerGlobalScope {
    constructor (worker) {
        ...
        // Per-worker IndexedDB instance.
        this.indexedDB = new FDBFactory();
        this.URL = URL;
...
class ServiceWorker extends events.EventEmitter {
    constructor (scriptURL, options) {
        ...
        const urlParts = nodeURL.parse(scriptURL);
    ...
    async _install () {
        const src = await this._getWorkerSource();
    ...
    async _getWorkerSource (options) {
...
class ServiceWorkerContainer {
    ...
    async register (scriptURL, options) {
        ...
        const worker = await new ServiceWorker(scriptURL, options)._install();
        const registration = new ServiceWorkerRegistration(worker, options, this);
        const domain = options.origin || nodeURL.parse(scriptURL).host;
    ...
    _unregister (sw) {
        const domain = nodeURL.parse(sw.scriptURL).host;
     ...
     */
    getRegistration (url) {
        const urlParts = nodeURL.parse(url);
     ...
     * @return undefined
     */
    x_clearDomain (domain) { // eslint-disable-line camelcase
